...
class FeedManager
{
	...
	{
		$feedId = 0;
		$resFeed = mysqli_query($this->db, "SELECT history.id,history.date,history.content,history.author AS author_id, user.name AS author FROM history LEFT JOIN user ON user.id=history.author WHERE history.id>'".$feedId."' ORDER BY history.date DESC LIMIT 2");
		while ($dataFeed = mysqli_fetch_assoc($resFeed))
		{
			require('views/feed.phtml');
		}
